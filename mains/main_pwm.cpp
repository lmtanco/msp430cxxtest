...
__interrupt void ISR_Timer1A0()
{
    ...
    // A los 2s se cambia los leds.
    if (counter == 0) {
      timer1.disable_interrupt(TIMER::TACCR0_CCIFG);
      port1.toggle_output(GPIO::BIT0);
      pwm_timer0_out1.stop_pwm();
      counter = 10;
    }
}
...
__interrupt void ISR_Port1(void)
{
    if (port1.is_interrupt_pending(GPIO::BIT3)) {
        ...
        pwm_timer0_out1.take_pin();
        pwm_timer0_out1.start_outmode7_up(11000,22000);
    }
}
...
void main(void)
{
    ...
    // Configure timer that will interrupt
    timer1.set_clock_source(TIMER::SMCLK, TIMER::_8);


    // Configure PWM
    pwm_timer0_out1.set_clock_source(TIMER::SMCLK, TIMER::_1);
